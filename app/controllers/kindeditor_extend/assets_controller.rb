#coding: utf-8
require "find"
class KindeditorExtend::AssetsController < ApplicationController
  skip_before_action :verify_authenticity_token
  def create
    @imgFile, @dir = params[:imgFile], params[:dir]
    unless @imgFile.nil?
      if KindeditorExtend::AssetUploader.save_upload_info? # save upload info into database
        begin
          @asset = "KindeditorExtend::#{@dir.camelize}".constantize.new(:asset => @imgFile)
        ...
        begin
          uploader = "KindeditorExtend::#{@dir.camelize}Uploader".constantize.new
  ...
  def list
    @root_path = "#{Rails.public_path}/#{RailsKindeditorExtend.upload_store_dir}/"
    @root_url = "/#{RailsKindeditorExtend.upload_store_dir}/"
    @img_ext = KindeditorExtend::AssetUploader::EXT_NAMES[:image]
    @dir = params[:dir].strip || ""
    unless KindeditorExtend::AssetUploader::EXT_NAMES.keys.map(&:to_s).push("").include?(@dir)
      render :plain => "Invalid Directory name."
      return
    end

    Dir.chdir(Rails.public_path)
    RailsKindeditorExtend.upload_store_dir.split('/').each do |dir|
